...
// "gin rummy free" had nothing of ours to land on, and every share was a bare "Faro" link with
...
for (const def of catalog) {
  ...
  const staticBody = `<h1>${escapeHtml(name)}</h1>
      ...
      <p><a href="/?g=${encodeURIComponent(id)}">Play ${escapeHtml(name)} on Faro</a></p>`;
