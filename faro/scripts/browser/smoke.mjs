...
const shots = process.argv[3] || '/tmp/faro-shots';
...
await page.evaluate(() => {
  const raw = JSON.parse(localStorage.getItem('faro.settings.v1') || '{}');
  localStorage.setItem('faro.settings.v1', JSON.stringify({ ...raw, botSpeed: 'instant' }));
  ...
  localStorage.setItem('faro.seenintro.v1', '1');
});
