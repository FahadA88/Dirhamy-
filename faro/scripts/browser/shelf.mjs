...
await p.evaluate(() => { localStorage.clear(); localStorage.setItem('faro.seenintro.v1', '1'); });
await p.goto(base, { waitUntil: 'networkidle' });
...
const followed = await p.evaluate(() => JSON.parse(localStorage.getItem('faro.follows.v1') || '[]'));
...
const stats = await p.evaluate(() => JSON.parse(localStorage.getItem('faro.builtinstats.v1') || '{}'));
